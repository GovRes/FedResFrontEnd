...
const renderWithContext = (ui: React.ReactElement) => {
  return render(
    <AllyContext.Provider value={mockContextValue}>{ui}</AllyContext.Provider>
  );
};
...
describe("Resume", () => {
  ...
  it("renders the resume input form", () => {
    ...
    expect(
      screen.getByText(
        /While we work on getting the ability to directly upload resumes/i
      )
    ).toBeInTheDocument();
  ...
  it("submits resume text and updates context", async () => {
    ...
    expect(mockSetStep).toHaveBeenCalledWith("usa-jobs");
  });
  ...
  it("shows fade animation class on containers", () => {
    ...
    const instructionText = screen.getByText(
      /While we work on getting the ability/i
    );
    const formContainer = screen
      .getByRole("textbox")
      .closest('div[class*="userChatContainer"]');
