...
> => {
  try {
    ...
    const errorResult = handleError("initialize", "client", error);
    return {
      success: false,
      error: `Authentication failed: ${errorResult.error}`,
      statusCode: 401,
    };
  }
};
...
export async function fetchUserRecord(
  ...
  try {
    ...
    if (errors && errors.length > 0) {
      ...
      const errorResult = handleError("fetch", "User", error, id);
...
> {
  ...
  try {
    ...
    if (errors && errors.length > 0) {
      const errorMessage = errors.map((e) => e.message).join(", ");
      const error = new Error(errorMessage);
      const errorResult = handleError("list", "Users", error);
...
export async function createUserRecord(
  ...
  try {
    ...
    if (errors && errors.length > 0) {
      const errorMessage = errors.map((e) => e.message).join(", ");
      const error = new Error(errorMessage);
      const errorResult = handleError("create", "User", error);
...
export async function updateUserRecord(
  ...
  try {
    ...
    if (errors && errors.length > 0) {
      const errorMessage = errors.map((e) => e.message).join(", ");
      const error = new Error(errorMessage);
      const errorResult = handleError("update", "User", error, id);
...
export async function deleteUserRecord(
  ...
  try {
    ...
    if (errors && errors.length > 0) {
      const errorMessage = errors.map((e) => e.message).join(", ");
      const error = new Error(errorMessage);
      const errorResult = handleError("delete", "User", error, id);
