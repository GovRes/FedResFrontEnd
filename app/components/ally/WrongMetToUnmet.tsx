...
export default function WrongMetToUnmet({
  ...
  let allyStatements = [
    `Our reviewer ${
      recommendation === "Do not recommend"
        ? "does not recommend"
        : "recommends"
    } that you apply for this job. We'll deal with this in a later version.`,
  ...
  const onSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    let objs: QualificationType[] = values
      .map((value) => {
        let item = qualifications.metQualifications.find(
          (obj) => obj.id === value
        );
        if (item) {
          let index = qualifications.metQualifications.indexOf(item);
          let updatedMetQualifications =
            qualifications.metQualifications.filter((_, i) => i !== index);
          setQualifications({
            ...qualifications,
            metQualifications: updatedMetQualifications,
          });
        }
        return item;
      })
  ...
  return (
    <div>
      <div className={`${styles.allyChatContainer}`}>
        ...
        >
          {qualifications.metQualifications.map(
