// Updated route.tsx - Fixed to properly use prompts and return correct schema responses
  ...
  try {
    ...
    if (content) {
      try {
        ...
        // Handle different schema types properly
        switch (schemaName) {
          case "topics":
            // Ensure we return the topics array
          ...
          case "pastJobs":
            // Ensure we return the pastJobs array
            if (parsedContent.pastJobs) {
              return new Response(
                JSON.stringify({ pastJobs: parsedContent.pastJobs }),
                {
                  status: 200,
                  headers: {
                    "Content-Type": "application/json",
            ...
            // Sometimes the AI might return the array directly
            else if (Array.isArray(parsedContent)) {
              return new Response(JSON.stringify({ pastJobs: parsedContent }), {
                status: 200,
                headers: {
                  "Content-Type": "application/json",
                  "X-Debug-Time": `${Date.now() - requestStartTime}ms`,
        ...
        // Fallback - return the parsed content as-is
        return new Response(JSON.stringify(parsedContent), {
          status: 200,
          headers: {
            "Content-Type": "application/json",
            "X-Debug-Time": `${Date.now() - requestStartTime}ms`,
            "X-Debug-Stage": "success-fallback",
          },
        });
      } catch (parseError) {
        ...
        try {
          ...
          if (jsonMatch) {
            ...
            // Handle schema-specific responses for extracted JSON too
            if (schemaName === "topics" && extractedJson.topics) {
              ...
              schemaName === "pastJobs" &&
              (extractedJson.pastJobs || Array.isArray(extractedJson))
            ) {
              const pastJobs = extractedJson.pastJobs || extractedJson;
